 ...
 * December 2017
 *
 * Note: modified printf statements are commented out
 * so that the program will run on an SD Card
 * Date: April 5th, 2018
 */
...
// when using Windows
#define CHARACTER_LCD_0_BASE 0x00001500
...
void PrintStringLCD( const char * str)
{
	...
	if(err == OS_ERR_NONE)
	{
		...
		if (err != OS_ERR_NONE)
		{
//			printf ("LCD::PrintString Error: %i", err);
		}
	} else {
