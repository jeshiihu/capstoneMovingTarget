...
#define PWM_ADDR 0x00000200
#define PWM_BASE FPGA_TO_HPS_LW_ADDR(PWM_ADDR)
...
{
    BSP_OS_TmrTickInit(OS_TICKS_PER_SEC);                       /* Configure and enable OS tick interrupt.              */
    //en_b en_a b_comp b a_comp a

    const uint8_t motorsOff = 0; //000000
    const uint8_t motorsOn = 48; // 110000

    const uint8_t motorStep1 = 58; //111010
    const uint8_t motorStep2 = 57; //111001
    const uint8_t motorStep3 = 53; //110101
    const uint8_t motorStep4 = 54; //110110
//
    uint8_t currStep = motorsOn;
    for(;;) {
    	BSP_WatchDog_Reset();                                /* Reset the watchdog.                                  */
    	alt_write_byte(PWM_BASE, currStep);
    	if(currStep == motorsOff)
    	{
    		currStep = motorsOn;
    	}
    	else if(currStep == motorsOn)
    	{
    		currStep = motorStep1;
    	}
    	else if(currStep == motorStep1)
    	{
    		currStep = motorStep2;
    	}
    	else if(currStep == motorStep2)
    	{
    		currStep = motorStep3;
    	}
    	else if(currStep == motorStep3)
    	{
    		currStep = motorStep4;
    	}
    	else if(currStep == motorStep4)
    	{
    		currStep = motorStep1;
    	}
