...
void InitMotor(enum Motor motor, INT32U freq)
{
	...
	if(motor == motorX)
		alt_write_word(PERIOD_BASE_X, numCycles);
	else
	...
	if(motor == motorX)
		alt_write_word(DUTY_BASE_X, duty);
	else
...
{
	if(motor == motorX)
		alt_write_hword(STEP_BASE_X, steps);
	else
		alt_write_hword(STEP_BASE_Y, steps);
}
...
{
	if(motor == motorX)
		alt_write_byte(DIR_BASE_X, (INT8U)dir);
	else
		alt_write_byte(DIR_BASE_Y, (INT8U)dir);
}
...
{
	if(motor == motorX)
		return (enum Direction)alt_read_byte(DIR_BASE_X);
	else
		return (enum Direction)alt_read_byte(DIR_BASE_Y);
}
...
INT32U GetFrequency(enum Motor motor)
{
	INT32U cycle = 0;
	if(motor == motorX)
		cycle = alt_read_word(PERIOD_BASE_X);
	else
		cycle = alt_read_word(PERIOD_BASE_Y);
...
{
	if(motor == motorX)
		return alt_read_word(DUTY_BASE_X);
	else
		return alt_read_word(DUTY_BASE_Y);
}
...
{
	if(motor == motorX)
	    return alt_read_hword(STEP_BASE_X);
	else
	    return alt_read_hword(STEP_BASE_Y);
}
