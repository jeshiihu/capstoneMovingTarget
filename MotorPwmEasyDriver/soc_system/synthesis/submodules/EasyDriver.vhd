...
entity EasyDriver is
	port (
		...
		period_read_n      : in  std_logic                     := '0';             -- avalon_period.read_n
		...
		duty_read_n        : in  std_logic                     := '0';             --   avalon_duty.read_n
...
begin
	write_to_hBridge:
	process(clk, reset_n,step_write_n, dir_write_n, enable_write_n, period_write_n, duty_write_n) is
		...
		begin
			...
			-- FPGA rising edge (50MHz) will produce PWM		
				...
				if(enable_write_n = '0') then -- check if duty has been written to
					conduit_end_enable  <= enable_writedata(0);
					enable_readdata     <= enable_writedata;	
				end if;
