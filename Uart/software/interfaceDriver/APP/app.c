...
CPU_STK uartTaskStk[TASK_STACK_SIZE];
...
int main ()
{
    ...
    os_err = OSTaskCreateExt((void (*)(void *)) uartTask,   /* Create the start task.                               */
                               (void          * ) 0,
                               (OS_STK        * )&uartTaskStk[TASK_STACK_SIZE - 1],
                               (INT8U           ) APP_TASK_PRIO,
    ...
    //create and allocate mem for msgQueue
    myQueue = OSQCreate(msgQueue, QUEUE_SIZE);
...
static void uartTask(void *p_arg)
{
	...
	struct ALT_16550_HANDLE_s uartHandle; //uses one of the built in UARTs on the HPS
	...
	for(;;){
		...
		status = alt_16550_fifo_read(&uartHandle, buffer, 16);
